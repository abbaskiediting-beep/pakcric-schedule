  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-6">
      ...
      >
        ...
        <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5 mt-8">
          <Info className="w-4 h-4 text-pak-green shrink-0" />
          <p className="text-xs text-neutral-400 leading-relaxed font-normal">
            Standings are determined by the <span className="text-white font-bold italic">Points Percentage (PCT)</span>. Click on table headers to <span className="text-pak-green">sort</span> the data.
          </p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.01] text-[10px] font-bold uppercase tracking-widest text-neutral-500 border-b border-white/5">
                <th className="p-6 cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('pos')}>
                  <div className="flex items-center gap-2">Pos {getSortIcon('pos')}</div>
                </th>
                <th className="p-6 cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('team')}>
                  <div className="flex items-center gap-2">Team {getSortIcon('team')}</div>
                </th>
                <th className="p-6 text-center cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('matches')}>
                  <div className="flex items-center justify-center gap-2">Mat {getSortIcon('matches')}</div>
                </th>
                <th className="p-6 text-center cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('won')}>
                  <div className="flex items-center justify-center gap-2">Won {getSortIcon('won')}</div>
                </th>
                <th className="p-6 text-center cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('lost')}>
                  <div className="flex items-center justify-center gap-2">Lost {getSortIcon('lost')}</div>
                </th>
                <th className="p-6 text-center cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('draw')}>
                  <div className="flex items-center justify-center gap-2">Draw {getSortIcon('draw')}</div>
                </th>
                <th className="p-6 text-center cursor-pointer hover:text-white transition-colors" onClick={() => requestSort('points')}>
                  <div className="flex items-center justify-center gap-2">Points {getSortIcon('points')}</div>
                </th>
                <th className="p-6 text-right text-white cursor-pointer hover:text-pak-green transition-colors" onClick={() => requestSort('pct')}>
            ...
            <tbody>
              {sortedData.map((team) => (
                <tr 
                  key={team.team}
                  className={`border-b border-white/5 hover:bg-white/[0.03] transition-colors ${team.team === 'Pakistan' ? 'bg-pak-green/5' : ''}`}
                >
                  <td className="p-6 font-mono font-bold text-neutral-500">{team.pos}</td>
                  <td className="p-6">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs ${team.team === 'Pakistan' ? 'bg-pak-green text-white' : 'bg-white/5 text-neutral-400'}`}>
                        {team.team.substring(0, 2).toUpperCase()}
                      </div>
                      <span className={`font-bold uppercase tracking-tight ${team.team === 'Pakistan' ? 'text-white' : 'text-neutral-300'}`}>
                        {team.team}
                      </span>
                    </div>
                  </td>
                  <td className="p-6 text-center font-normal">{team.matches}</td>
                  ...
                  <td className="p-6 text-center font-bold text-white">{team.points}</td>
                  <td className="p-6 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-lg font-display font-bold text-white leading-none mb-1">{team.pct.toFixed(2)}%</span>
