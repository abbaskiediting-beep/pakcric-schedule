    ...
    <div className="max-w-7xl mx-auto py-12 px-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-12">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <h1 className="text-3xl md:text-5xl font-display font-bold uppercase tracking-tight mb-6 text-white">
          ...
          <p className="text-ink/40 font-bold uppercase tracking-widest text-[10px] border-l-2 border-pak-green pl-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {players.slice(0, 3).map((player, idx) => (
          <motion.div 
            ...
            className="bg-card-bg border border-card-border rounded-[32px] p-6 relative overflow-hidden group"
          >
            ...
            <div className="flex items-center gap-6 relative z-10">
              ...
              <div className="min-w-0">
                ...
                <h3 className="text-lg font-display font-bold text-white uppercase truncate">{player.name}</h3>
                <div className="flex gap-4 mt-2">
                  <div>
                    <p className="text-[8px] font-bold text-neutral-500 uppercase">Runs</p>
                    <p className="text-sm font-bold text-white tabular-nums">{player.stats.runs || '—'}</p>
                  </div>
                  <div>
                    <p className="text-[8px] font-bold text-neutral-500 uppercase">Avg</p>
                    <p className="text-sm font-bold text-white tabular-nums">{player.stats.avg}</p>
                  </div>
                </div>
              </div>
            </div>
            <Link 
              ...
              className="mt-6 flex items-center justify-center gap-2 w-full py-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl text-[9px] font-bold uppercase tracking-widest text-ink/60 hover:text-white transition-all"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
        {/* Batting Stats Intro */}
        <section className="bg-card-bg border border-card-border rounded-[40px] p-10 relative overflow-hidden group hover:border-pak-green/20 transition-all">
          ...
          <div className="space-y-4 text-ink/60 font-medium leading-relaxed">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {['Total Runs', 'Avg', 'SR', '50s/100s'].map(t => (
                <span key={t} className="px-3 py-1 bg-white/5 rounded-lg text-[9px] font-bold text-white/40 uppercase tracking-widest">{t}</span>
              ))}
            </div>
          </div>
        </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-12">
        {[
          ...
          { label: 'Fastest Strike Rate', player: 'Mohammad Rizwan', value: '129.12', icon: <Zap className="w-4 h-4 text-pak-green" /> },
          { label: 'Top Wicket Taker', player: 'Shaheen Afridi', value: '139', icon: <Target className="w-4 h-4 text-blue-400" /> },
          { label: 'Consistent Leader', player: 'Noman Ali', value: '24.52', icon: <Activity className="w-4 h-4 text-purple-400" /> }
        ].map((feat, i) => (
          <div key={i} className="bg-card-bg border border-card-border p-6 rounded-[32px]">
             <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center">
                  {feat.icon}
                </div>
                <div className="min-w-0">
                  <p className="text-[8px] font-bold text-neutral-500 uppercase tracking-widest">{feat.label}</p>
                  <p className="text-xs font-bold text-white truncate">{feat.player}</p>
                </div>
             </div>
             <p className="text-3xl font-display font-bold text-white tabular-nums">{feat.value}</p>
          </div>
        ))}
      </div>
    </div>
