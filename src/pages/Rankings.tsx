...
export default function Rankings() {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 sm:py-12 px-4 sm:px-6">
      ...
      <AdPlaceholder type="leaderboard" className="mb-12 md:mb-16" />

      {/* Team Rankings Section */}
      <section className="mb-16 md:mb-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 md:mb-12">
          <div>
            <h2 className="text-3xl md:text-5xl font-display font-bold uppercase tracking-tight text-white mb-3 md:mb-4">
            ...
            <Link to={`/icc-${card.type.toLowerCase()}-ranking-2026`} key={card.type} className="block h-full">
              ...
              >
                ...
                <div className="absolute -top-12 -right-12 opacity-5 pointer-events-none group-hover:scale-110 transition-transform duration-1000">
                   <Trophy className="w-48 md:w-64 h-48 md:h-64" />
                </div>
              </motion.div>
            </Link>
          ))}
      ...
      <section className="mb-16 md:mb-24">
        <div className="flex items-center gap-4 mb-6 md:mb-8">
          <div className="h-2 w-8 md:w-12 bg-pak-green rounded-full" />
          <h2 className="text-2xl md:text-3xl font-display font-bold uppercase tracking-tight text-white italic">Major Championship Pursuit</h2>
        </div>
        ...
        <Link to="/icc-wtc-projections-2026">
          <motion.div
            ...
            className="bg-card-bg border border-card-border rounded-3xl md:rounded-[48px] p-6 sm:p-8 md:p-16 mb-8 relative overflow-hidden group cursor-pointer hover:border-pak-green/30 transition-all shadow-3xl"
          >
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
              <div>
                <div className="inline-block px-3 md:px-4 py-1.5 bg-pak-green/10 text-pak-green rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-[2px] md:tracking-[3px] mb-6 md:mb-8 border border-pak-green/20">
                  WTC Standings 2025-27
                </div>
                <h2 className="text-3xl md:text-6xl font-display font-bold text-white uppercase tracking-tighter leading-tight mb-6 md:mb-8">
                ...
                <div className="space-y-4 md:space-y-6 mb-8 md:mb-10">
                  <p className="text-sm md:text-lg text-ink/60 font-medium leading-relaxed max-w-lg">
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-2xl p-5 md:p-6">
                    <p className="text-[13px] md:text-sm font-bold text-white mb-2">Current Standing: <span className="text-pak-green">5th Position</span></p>
                    <p className="text-[11px] md:text-xs text-ink/40 font-medium leading-relaxed">Pakistan needs to win their upcoming home series against England and Bangladesh to climb the ladder.</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-[9px] md:text-[10px] font-bold uppercase tracking-[0.3em] md:tracking-[0.4em] text-pak-green group-hover:gap-6 transition-all">
                  Analyze Full Points Table <ChevronRight className="w-4 h-4 md:w-5 md:h-5" />
                </div>
              </div>
              <div className="hidden md:flex justify-end relative">
                <div className="w-64 h-64 md:w-80 md:h-80 rounded-[40px] md:rounded-[60px] bg-pak-green/5 border border-pak-green/10 flex items-center justify-center relative rotate-3 group-hover:rotate-0 transition-transform duration-700">
                    <Trophy className="w-32 h-32 md:w-40 md:h-40 text-pak-green opacity-30 group-hover:opacity-100 group-hover:scale-110 transition-all duration-700" />
                    <div className="absolute -bottom-6 -right-6 w-24 h-24 md:w-32 md:h-32 bg-pak-green/20 rounded-full blur-[40px] md:blur-[60px]" />
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <section className="mb-16 md:mb-24">
        <div className="max-w-2xl mb-8 md:mb-12">
          <h2 className="text-3xl md:text-5xl font-display font-bold uppercase tracking-tight text-white mb-4 md:mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-16">
          {[
            ...
            <div key={format.title} className={`bg-gradient-to-br ${format.color} border border-card-border rounded-3xl md:rounded-[40px] p-8 md:p-10 shadow-2xl flex flex-col`}>
               <h3 className="text-xl md:text-2xl font-display font-bold uppercase mb-3 md:mb-4 text-white">{format.title}</h3>
               <p className="text-[11px] md:text-xs text-white/40 font-medium mb-6 md:mb-8 leading-relaxed italic">{format.desc}</p>
               <div className="space-y-3 flex-1">
                  {[
                    ...
                    <Link 
                      ...
                      className="flex items-center justify-between p-4 md:p-5 bg-black/40 border border-white/5 rounded-xl md:rounded-2xl hover:bg-white hover:text-black transition-all group/link shadow-sm"
                    >
                      <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.2em]">{cat.name} Rankings</span>
      ...
      <section className="mb-16 md:mb-24 text-white">
        <div className="bg-white/[0.02] border border-card-border rounded-3xl md:rounded-[48px] p-6 sm:p-10 md:p-20 relative overflow-hidden group">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center relative z-10">
            <div>
              <h2 className="text-2xl md:text-5xl font-display font-bold uppercase tracking-tight text-white mb-6 md:mb-8 text-white">
                How ICC <span className="text-pak-green">Rankings Work</span>
              </h2>
          <div className="space-y-4 md:space-y-6 text-sm md:text-base text-ink/70 font-medium leading-relaxed text-white/70">
            <p>
              ...
              {[
                ...
                <div key={idx} className="flex gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-[13px] md:text-sm opacity-60 block leading-snug">{item.desc}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-pak-green/10 p-8 md:p-12 rounded-2xl md:rounded-[40px] border border-pak-green/20">
               <TrendingUp className="w-12 h-12 md:w-20 md:h-20 text-pak-green mb-4 md:mb-6 opacity-20" />
               <h3 className="text-lg md:text-xl font-display font-bold text-white uppercase mb-3 md:mb-4">Why Rankings Are Important</h3>
               <p className="text-sm text-white/60 font-medium leading-relaxed mb-6 md:mb-8">
               ...
               <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <div className="text-center p-3 md:p-4 bg-black/20 rounded-xl md:rounded-2xl">
                     <span className="text-xl md:text-2xl font-bold text-white block leading-none mb-1 md:mb-2">Form</span>
                     <span className="text-[7px] md:text-[8px] font-bold text-pak-green uppercase tracking-widest">Team Growth</span>
                  </div>
                  <div className="text-center p-3 md:p-4 bg-black/20 rounded-xl md:rounded-2xl">
                     <span className="text-xl md:text-2xl font-bold text-white block leading-none mb-1 md:mb-2">Status</span>
                     <span className="text-[7px] md:text-[8px] font-bold text-pak-green uppercase tracking-widest">World Standing</span>
                  </div>
               </div>
            </div>
          </div>
          <div className="absolute top-0 right-0 w-64 h-64 bg-pak-green/5 blur-[80px] md:blur-[120px] rounded-full translate-x-1/2 -translate-y-1/2" />
        </div>
      </section>

      {/* Latest Rankings Update section */}
      <section className="mb-16 md:mb-24 text-center max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-display font-bold uppercase tracking-tight text-white mb-4 md:mb-6">
          Latest Rankings <span className="text-pak-green">Updates</span>
        </h2>
        <p className="text-sm md:text-lg text-ink/60 font-medium leading-relaxed mb-8 md:mb-10">
        ...
        <div className="overflow-x-auto rounded-2xl md:rounded-[32px] border border-white/5 bg-white/5 custom-scrollbar">
          <table className="w-full text-left min-w-[500px]">
            <thead className="bg-white/10">
              <tr>
                <th className="px-6 md:px-8 py-3 md:py-4 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-white">Format</th>
                <th className="px-6 md:px-8 py-3 md:py-4 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-white">Pakistan Ranking</th>
                <th className="px-6 md:px-8 py-3 md:py-4 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-white">Current Rating</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {[
                ...
                <tr key={row.format} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 md:px-8 py-4 text-[11px] md:text-xs font-bold text-white/80">{row.format}</td>
                  <td className="px-6 md:px-8 py-4 text-[11px] md:text-xs font-bold text-pak-green">{row.rank}</td>
                  <td className="px-6 md:px-8 py-4 text-[11px] md:text-xs font-medium text-white/40">{row.points} Points</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="mb-16 md:mb-20">
        <div className="bg-card-bg border border-card-border rounded-3xl md:rounded-[48px] p-8 md:p-16 text-center shadow-3xl shadow-black/40">
           <Star className="w-8 md:w-12 h-8 md:h-12 text-pak-green mx-auto mb-6 md:mb-8 animate-pulse" />
           <h2 className="text-2xl md:text-5xl font-display font-bold uppercase tracking-tighter text-white mb-4 md:mb-6">Explore More <span className="text-pak-green">Cricket Insights</span></h2>
           <p className="text-sm md:text-lg text-ink/60 font-medium max-w-2xl mx-auto mb-8 md:mb-10 leading-relaxed">
             Want more detailed information? Follow Pakistan's journey as they climb the ladders across all formats.
           </p>
           <div className="flex flex-col sm:flex-row justify-center gap-4 md:gap-6">
              <Link to="/schedule" className="w-full sm:w-auto px-10 py-4 bg-pak-green text-white rounded-2xl text-[10px] md:text-[11px] font-bold uppercase tracking-[3px] md:tracking-[4px] hover:shadow-pak-green/30 hover:shadow-2xl transition-all active:scale-95">Check Schedule</Link>
              <Link to="/news" className="w-full sm:w-auto px-10 py-4 bg-white/5 border border-white/20 text-white rounded-2xl text-[10px] md:text-[11px] font-bold uppercase tracking-[3px] md:tracking-[4px] hover:bg-white hover:text-black transition-all active:scale-95">Read Latest News</Link>
           </div>
           <div className="mt-12 md:mt-16 flex flex-wrap justify-center gap-4 md:gap-8 border-t border-white/5 pt-8 md:pt-10">
              <Link to="/squads" className="text-[9px] md:text-[10px] font-bold uppercase tracking-[2px] md:tracking-[3px] text-ink/40 hover:text-pak-green transition-colors">Squad Updates</Link>
              <Link to="/players-stats" className="text-[9px] md:text-[10px] font-bold uppercase tracking-[2px] md:tracking-[3px] text-ink/40 hover:text-pak-green transition-colors">Player Stats Hub</Link>
              <Link to="/" className="text-[9px] md:text-[10px] font-bold uppercase tracking-[2px] md:tracking-[3px] text-ink/40 hover:text-pak-green transition-colors">Home Dashboard</Link>
           </div>
        </div>
      </section>
    </div>
