...
export default function MatchCard({ match, index }: MatchCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-card-bg border border-card-border rounded-2xl p-5 flex flex-col justify-between cursor-pointer transition-all duration-300 group"
      id={`match-${match.id}`}
    >
      <div>
        <div className="flex justify-between items-start mb-4">
           <span className="text-[10px] font-bold text-white/40 group-hover:text-pak-green uppercase tracking-widest transition-colors">{match.series}</span>
           <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-tighter border ${formatColor} transition-all group-hover:scale-105`}>
            {match.format}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between mb-4 px-2">
          ...
          <div className="text-[10px] font-bold text-neutral-600 group-hover:text-pak-green/60 transition-colors italic">VS</div>

          <div className="flex items-center gap-2">
             <span className="text-xs font-bold uppercase group-hover:text-white transition-colors">{match.opponent.substring(0, 3)}</span>
             <div className="w-8 h-8 rounded-full border border-card-border group-hover:border-pak-green group-hover:ring-2 group-hover:ring-pak-green/30 bg-black/20 p-1 overflow-hidden shrink-0 transition-all duration-300">
                <img 
